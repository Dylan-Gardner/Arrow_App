    ...
    this.interval = setInterval(()=>{
      try{
        remote.getPlayerState().then(player => {
          this.props.songUpdate(player.track.name, player.track.artist.name, player.track.duration, player.isPaused, player.playbackPosition)
        });
      }catch(err){
...
const mapDispatchToProps = (dispatch) => {
  ...
  return {
    // Increase Counter
    songUpdate: (trackName, artistName, trackLength, isPaused, position) => {dispatch(songUpdate(trackName, artistName, trackLength, isPaused, position))},
  };
};
